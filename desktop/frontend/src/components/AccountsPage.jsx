...
function AccountRow({ account, onDelete, onUpdate }) {
  ...
  return <div className="gateway-table-row"><div><strong>{account.name || account.email || `账号 ${account.id}`}</strong><small>{account.email || account.user_id || account.provider}</small></div><label className="compact-toggle"><input checked={draft.enabled} onChange={(e) => setDraft((value) => ({ ...value, enabled: e.target.checked }))} type="checkbox" /><span>{draft.enabled ? '已启用' : '已停用'}</span></label><div className={`account-health-state is-${health}`} title={account.health_error || healthDetail}><span><i />{healthText}</span><small>{account.health_error || healthDetail}</small></div><input className="concurrency-input" max="64" min="1" onChange={(e) => setDraft((value) => ({ ...value, max_concurrent: Number(e.target.value) || 1 }))} type="number" value={draft.max_concurrent} /><div className="row-actions"><button className="icon-button bordered" onClick={save} title="保存账号" type="button"><Save size={14} /></button><button className="icon-button bordered danger-icon" onClick={() => onDelete(account.id)} title="删除账号" type="button"><Trash2 size={14} /></button></div></div>;
}
