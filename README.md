...
- Turnstile：Linux/macOS 默认 **Playwright + CloakBrowser**；Windows 默认 Go BrowserPool；可选 lite farm
...
| Python 3.10+ + venv | Linux/macOS 默认 Turnstile 路径 | Windows/`go-browser` 不需要 |
| Playwright + CloakBrowser | Linux/macOS 默认无头浏览器方案 | Windows/`go-browser` 不需要 Playwright |
...
sudo mkdir -p /opt
cd /opt
sudo git clone <your-repository-url> /opt/GRF
cd /opt/GRF
...
sudo /opt/cloakbrowser-venv/bin/pip install -r /opt/GRF/scripts/requirements-turnstile.txt
...
# 或：/opt/GRF/scripts/turnstile_mint.py
...
Windows 默认使用 Go 原生 BrowserPool，不要求 Python 或 Playwright。需要安装：

- Go（加入 `PATH`）
- Google Chrome，或通过 `CHROME_PATH` 指定 Chromium
- Docker Desktop（仅在使用仓库内置 clearance 栈时需要）
...
脚本默认先执行全部测试，再生成 `bin\grf.exe`；开发时可使用 `-SkipTests`。Windows 产物使用 `grf.exe`，避免与其他 `grok.exe` 程序冲突。Windows 用户可以在 `config.env` 中显式写入：

```env
TURNSTILE_PROVIDER=go-browser
...
桌面端采用 Wails v3 + React，提供总览、实时日志、运行产物和常用设置四个工作区。Windows 需要：

- Go（版本以 `go.mod` 为准）
- Node.js 20+（当前前端构建链兼容 Node.js 22.10）
- Microsoft Edge WebView2 Runtime（Windows 10/11 通常已内置）
- Google Chrome 或 Chromium（用于原生 Go BrowserPool）
...
`warp-proxy` 需要容器 `NET_ADMIN` 能力。如果 WARP 容器在 Docker Desktop 中不可用，可以改用外部 HTTP 代理并设置 `REGISTER_PROXY`、`HTTP_PROXY` 和 `HTTPS_PROXY`。
...
`go-browser` 为每个 worker 保持一个 Chromium 进程，每次 mint 创建新的隔离 browser context；若当前 Chromium 不支持该 CDP 能力，则自动回退为新 tab 并在导航前清理 Cookie、缓存和目标 origin 的存储。迁移验证期间，Linux/macOS 默认 `browser` 仍使用 Python 路径，便于进行 A/B 对比和快速回滚。实现与验收步骤见 [`TURNSTILE_GO_MIGRATION_PLAN.md`](TURNSTILE_GO_MIGRATION_PLAN.md)。

Windows 下 `browser` 默认直接映射到 `go-browser`，无需 Python；如需显式测试 Python 路径，可以设置 `TURNSTILE_PROVIDER=playwright`。
